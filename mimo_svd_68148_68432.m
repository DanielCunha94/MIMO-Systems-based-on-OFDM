function trab2_mimo_svd_68148_68432
%CSF 18/19
...
PR=1;
N_bits = Nc*2;
ber_s1 = zeros(1,length(N_OFDM_SYM ));
ber_s2 = ber_s1;
ber_mean = ber_s1;
ber_each_s1 = zeros(1,length(EbN0));
ber_each_s2= ber_each_s1;
ber_each_mean= ber_each_s1;
...
for p=1:length(EbN0)    % EbN0_dB range
    ...
    for k=1:N_OFDM_SYM   % number of OFDM symbols to be simulated
        ...
        data2 = data_gen(N_bits);
        %data modulation
        data_mod1 = mod_data(data,m);
        data_mod2 = mod_data(data2,m);
        data_mod = [data_mod1 ; data_mod2];
        %################# Channel Model #################
        %awgn noise
        var = PR*10.^(-EbN0(p)/10)/log2(m);
        Noise = sqrt(var/2).*(randn(2,Nc) + 1j*randn(2,Nc)) ;
        %multipath channel
        ...
        H11f = H11f(129:Nc+128);
        H21f = H21f(129:Nc+128);
        H12f = H12f(129:Nc+128);
        H22f = H22f(129:Nc+128);
        
        H = [H11f;H12f;H21f;H22f];
        X = svd_tx(H,data_mod,Nc);
        %################# Receiver ######################
        % Equalization:
        S=svd_rx(H,X,Noise,Nc);
        % Data demodulation
        demod_data1 = demod_data(S(1,:),m,N_bits);
        demod_data2 = demod_data(S(2,:),m,N_bits);
        % BER computation
        ber_s1(k) = compute_ber(demod_data1,data);
        ber_s2(k) = compute_ber(demod_data2,data2);
        ber_mean(k) = (ber_s1(k)+ber_s2(k))/2;
    end
    ber_each_s1(p) = mean(ber_s1);
    ber_each_s2(p) = mean(ber_s2);
    ber_each_mean(p) = mean(ber_mean);
end
figure(1),
semilogy(EbN0,ber_each_s1,'-or',EbN0,ber_each_s2,'-ob',EbN0,ber_each_mean,'-k');
axis([0 20 10^-5 0.5]),grid on,xlabel('EbN0 (dB)'),ylabel('BER'),
legend('2x2 MIMO S1','2x2 MIMO S2','2x2 MIMO Average');
[~,ber_each_ebeq,~] = trab1(EbN0);
ber_each_freq21 =trab2_prt1(EbN0);
figure(2),
semilogy(EbN0,ber_each_ebeq,'-or',EbN0,ber_each_freq21,'-ob',EbN0,ber_each_mean,'-k');
axis([0 20 10^-5 0.5]),grid on,xlabel('EbN0 (dB)'),ylabel('BER'),
legend('SISO-MRC OFDM','2x1 ALAMOUTI OFDM','2x2 MIMO Average');
end
%%%%%%%%%%%%%%% Auxiliary Functions %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for n=1:Npaths
    pot_componente=0.5;
    multipath(n)=sqrt(pot_componente)*randn(1,1)+1i*sqrt(pot_componente)*randn(1,1);
...
%****************svd_tx ******************
function X=svd_tx(H,mod_data,Nc)
X= zeros(2,Nc);
for n=1:1:768
    H22_tx = reshape(H(:,n),2,2);
    [~,~,V_tx] = svd(H22_tx);  % V = W
    X(:,n) = V_tx * mod_data(:,n);
end
end
%****************svd_rx ******************
function S=svd_rx(H,X,noise,Nc)
Y= zeros(2,Nc);
S= zeros(2,Nc);
for o=1:1:768
    H22_rx = reshape(H(:,o),2,2);
    [U_rx,~,~] = svd(H22_rx);  % G = U^H
    Y(:,o) = H22_rx * X(:,o) + noise(:,o);
    S(:,o) = U_rx' * Y(:,o);
end
end
...
PR=1;
N_bits = Nc*2;
ber_ofdmeq = zeros(1,length(N_OFDM_SYM ));
ber_ofdm = zeros(1,length(N_OFDM_SYM ));
ber_ofdmf = zeros(1,length(N_OFDM_SYM ));
ber_each_eb = zeros(1,length(EbN0));
ber_each_ebeq = zeros(1,length(EbN0));
ber_each_ebf = zeros(1,length(EbN0));
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
for p=1:length(EbN0)    % EbN0_dB range
...
function ber_each_freq21 =trab2_prt1 (EbN0)
%CSF 18/19
% Daniel Cunha 68148 & Tiago Marques 68432
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Reference ofdm chain
